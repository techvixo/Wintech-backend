...
const createCertificate = async (req, res) => {
  ...
  if (req.files && req.files.length > 0) {
    const imagePath = await fileUploader(req.files, `certificate-image-${certificateData.name_en}`, 'image');
    certificateData.image = imagePath;
  }

  const certificate = await certificateServices.createCertificate(certificateData);
  if (!certificate) {
...
const updateSpecificCertificate = async (req, res) => {
  ...
  if (req.files && req.files.length > 0) {
    const imagePath = await fileUploader(req.files, `certificate-image-${updateData.name_en}`, 'image');
    updateData.image = imagePath;
  }

  const certificate = await certificateServices.updateSpecificCertificate(id, updateData);
